...
def getDistanceBoxClosestToPoint(boxes, center):
    ...
    for i in range(len(boxes[0][0])):
        if getDistance([int((boxes[0][i][0] + boxes[0][i][3]) / 2), int((boxes[0][i][1] + boxes[0][i][2]) / 2)],
                       center) < min:
            min = getDistance([int((boxes[0][i][0] + boxes[0][i][3]) / 2), int((boxes[0][i][1] + boxes[0][i][2]) / 2)],
                              center)
    return min
    ...
    with tf.Session(graph=detection_graph, config=session_config) as sess:
        ...
        # for i in list:
            ...
            if cv_check is True:
                ...
                if getDistanceBoxClosestToPoint(boxes, center_contour) <= 400:
                    write_cv = False
                    ...
                    for cnt in contours:
                        ...
                        if area > 2000 and area < 12000:
                            if 3 <= len(approx) <= 10:
                                ...
                                cv_check = False
                # cv2.imwrite(
                #     'C:\\Users\\purva\\Documents\\model\\metric_test_set_1\\results_deep_learning_and_cv_v2\\result' + '_' + str(i) + '.jpg',
                #     image_np)
        ...
        numpy.savetxt('C:\\Users\\purva\\Documents\\model\\deltas_mixed_detection_v2.csv', a, delimiter=",")
        for i in deltas:
            total = total + i
